...
if ( ! function_exists('show_like_button'))
{
	function show_like_button($id='')
	{
		...
		{
			$row = $query->row();
			echo '<a href="javascript:void(0);" url="'.site_url('user/like/'.$id).'" class="like-button">لایک</a>('.$row->like.')';
		}
	}
}
...
if ( ! function_exists('show_dislike_button'))
{
	function show_dislike_button($id='')
	{
		...
		{
			$row = $query->row();
			echo '<a href="javascript:void(0);" url="'.site_url('user/dislike/'.$id).'" class="dislike-button">عدم پسندیدن</a>('.$row->dislike.')';
		}
	}
}
