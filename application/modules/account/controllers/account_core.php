            ...
            if ($query->num_rows() > 0) {
                ...
                if ($row->banned == 1) {
                    ...
                    $msg = '<div class="alert alert-danger">
					        	...
					        	<strong>کاربر ممنوع شده است</strong>
                    ...
                    $msg = '<div class="alert alert-danger">
					        	...
					        	<strong>حساب غیر فعال است</strong>
...
//						create_log($row->user_name);
                ...
                $msg = '<div class="alert alert-danger">
					        ...
					        <strong>رمز عبور یا نام کاربری نامعتبر است</strong>
    ...
    public function terms_check($str) {
        ...
        if ($_POST['terms_conditon'] == '') {
            $this->form_validation->set_message('terms_check', 'شما باید توافقنامه را قبول نمایید');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function useremail_check($str) {
        ...
        if ($res > 0) {
            $this->form_validation->set_message('useremail_check', 'ایمیل هم اکنون در حال استفاده توسط شخص دیگر است');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function username_check($str) {
        ...
        if ($res > 0) {
            $this->form_validation->set_message('username_check', 'نام کاربری هم اکنون در حال استفاده توسط شخص دیگر است');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function check_email_validation($str) {
        ...
        if ($res <= 0) {
            $this->form_validation->set_message('check_email_validation', 'ایمیل یافت نگردید');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function requestconfirmation() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->send_confirmation_email($userdata);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">ایمیل تایید ارسال گردید، لطفا عملیات را پیگیری نمایید</div>');
    ...
    public function confirm($email = '', $code = '') {
        ...
        if ($res == TRUE) {
            $this->session->set_flashdata('msg', '<div class="alert alert-success">ایمیل تایید گردید، هم اکنون میتوانید وارد سیستم شوید</div>');
            redirect(site_url('account/showmsg'));
        } else {
            $this->session->set_flashdata('msg', '<div class="alert alert-danger">تایید نامعتبر بود، لطفا مجددا سعی نمایید</div>');
            redirect(site_url('account/showmsg'));
        }
    }
    ...
    function currentpass_check($str) {
        ...
        if ($res <= 0) {
            $this->form_validation->set_message('currentpass_check', 'رمز عبور فعلی نامعتبر است');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    function update_password() {
        if ($this->session->userdata('recovery') != 'yes')
            $this->form_validation->set_rules('current_password', 'رمز عبور فعلی', 'required|callback_currentpass_check');
        $this->form_validation->set_rules('new_password', 'رمز جدید', 'required|matches[re_password]');
        $this->form_validation->set_rules('re_password', 'تکرار رمز جدید', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">رمز عبور با موفقیت تغییر یافت</div>');
    ...
    function recoverpassword() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">ایمیل خواسته شده برای شما ارسال گردید.</div>');
            redirect(site_url('account/showmsg'));
        }
    }
    ...
    function useremail_match_check($str) {
        ...
        if ($res <= 0) {
            ...
            $this->form_validation->set_message('useremail_match_check', 'ایمیل مطابقت ندارد');

            return FALSE;
        } else if (is_banned($str)) {

            $this->form_validation->set_message('useremail_match_check', 'کاربر ممنوع شده است');
    ...
    function useremail_user_ban_check($str) {
        ...
        if (is_banned($str)) {
            ...
            $this->form_validation->set_message('useremail_user_ban_check', 'کاربر ممنوع شده است');
    ...
    function resetpassword($user_name = '', $recovery_key = '') {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-block">لینک تغییر رمز عبور نامعتبر است</div>');
