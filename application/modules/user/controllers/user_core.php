...
class User_core extends CI_Controller {
	...
	public function updatepost()
	{
		...
		else
		{
			...
			if($res['error']==1)
			$this->session->set_flashdata('msg', '<div class="alert alert-error">پست به روز نگردید</div>');
			else	
			$this->session->set_flashdata('msg', '<div class="alert alert-success">پست به روز گردید <a href="">اینجا</a> to publish it.</div>');
