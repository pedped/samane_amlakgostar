...
class Package_core extends CI_Controller {
	...
	function addpackage()
	{
		...
		else
		{
			...
			if($id>0) {
				...
				$this->session->set_flashdata('msg', '<div class="alert alert-success">پکیج جدید ساخته شد</div>');
			}
			else {

				$this->session->set_flashdata('msg', '<div class="alert alert-success">خطا در اضافه کردن پکیج</div>');	
			}
	...
	public function remove_package($id) {
		...
		$this->session->set_flashdata('msg', '<div class="alert alert-success">بسته حذف گردید</div>');
		redirect(site_url('admin/package/all'));
	}
	...
	public function remove_bulk_packages()
	{
		...
		$this->session->set_flashdata('msg', '<div class="alert alert-success">بسته ها حذف گردیدند</div>');
		redirect(site_url('admin/package/all'));			
	}
	...
	public function updatepackage()
	{
		...
		else
		{
			...
			$this->package_model->update_package($data,$id);
			$this->session->set_flashdata('msg', '<div class="alert alert-success">بسته ها به روز گردیدند</div>');
