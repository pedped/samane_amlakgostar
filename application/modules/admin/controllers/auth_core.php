...
class Auth_core extends MX_Controller {
    ...
    function recoverpassword() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">ایمیل برای شما ارسال گردید، لطفا چک نمایید</div>');
    ...
    function resetpassword($user_name = '', $recovery_key = '') {
        ...
        if ($res > 0) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-block">لینک تغییر رمز نامعتبر است</div>');
            redirect(site_url('admin/auth/forgotpass'));
        }
    }
    ...
    function login() {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($ok == 1) {
                ...
                $data = array('error' => '<div class="alert alert-danger" style="margin-top:10px;">حساب تاید نشده است</div>');
                $this->load->view('admin/login_view', $data);
            }
        } else {
            $data = array('error' => '<div class="alert alert-danger" style="margin-top:10px;">ورود ناموفق بود</div>');
            $this->load->view('admin/login_view', $data);
        }
    }
    ...
    function useremail_check_signup($str) {
        $res = $this->auth_model->is_email_exists($str);
        if ($res > 0) {
            $this->form_validation->set_message('useremail_check_signup', 'ایمیل هم اکنون ثبت شده است');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    function username_check_signup($str) {
        ...
        if ($res > 0) {
            $this->form_validation->set_message('username_check_signup', 'نام کاربری هم اکنون ثبت شده است');
            return FALSE;
        } else {
            return TRUE;
        }
