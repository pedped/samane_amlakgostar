    ...
    public function add() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->input->post('action_type') == 'update') {
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success">صفحه به روز گردید</div>');
            } else {
                $id = $this->page_model->insert_page($data);
                $this->session->set_flashdata('msg', '<div class="alert alert-success">صفحه اضافه گردید</div>');
            }
    ...
    public function delete($page = '0', $id = '', $confirmation = '') {
        if ($confirmation == '') {
            ...
            if ($confirmation == 'yes') {
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success">سفحه حذف گردید</div>');
            }
